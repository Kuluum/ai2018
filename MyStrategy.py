...
class MyStrategy:
    ...
    def custom_rendering(self):
        ...
        for point in predictions:
            ...
            ataker_pos = Vector3D(self.ataker.x, self.ataker.z, self.ataker.y)
    ...
    def act_attacker(self, me: Robot, rules: Rules, game: Game, action: Action):
        ...
        gate_z_target = rules.arena.depth + rules.arena.goal_depth
        ...
        if self.should_hit:
            ...
            ball_to_center_point = Vector2D(ball_curr_x - gate_center_direction.x * game.ball.radius,
                                            ball_curr_z - gate_center_direction.z * game.ball.radius)
            delta_pos = ball_to_center_point - me
            self.ataker_target = Vector3D(ball_to_center_point.x, ball_to_center_point.z, 0)
            ...
            sec = 1
            tik = 60
            prediction = self.predict_ball(sec, tik)
            tiks_pass = 0
            for point in prediction:
                tiks_pass += 1
                if point.y > self.game.ball.radius * 2:
                    continue
                target_x = point.x
                if target_x - me.x < 0:
                ...
                target_z = point.z - game.ball.radius * 5

                target = Vector3D(target_x, target_z, 1)

                delta_pos = target-Vector3D(me.x, me.z, me.y)
                need_speed = max(delta_pos.len() / float(tiks_pass), ROBOT_MAX_GROUND_SPEED*0.5)
            ...
            target_x = ball_curr_x
            if target_x - me.x < 0:
                target_x += game.ball.radius * 2
            else:
                target_x -= game.ball.radius * 2

            action.target_velocity_x = target_x * ROBOT_MAX_GROUND_SPEED * 0.1
            action.target_velocity_y = 0.0
